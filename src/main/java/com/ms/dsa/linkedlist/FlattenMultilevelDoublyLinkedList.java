...
public class FlattenMultilevelDoublyLinkedList {
    ...
    public Node flatten(Node head) {
        Node curr = head, next;
        while (curr != null) {
            if (curr.child != null) { // flatten it to the next level
                next = curr.next; // store the current node next to prevent loss of access
                curr.child.prev = curr; // make the child prev to curr
                curr.next = curr.child; // curr next to it's child
                Node temp = curr.child;
                curr.child = null; // break the curr & it's child link
                while (temp.next != null) { // In second level traverse till last node.
                    temp = temp.next;
                }
                temp.next = next; // making link with the actual next of curr & last node of the 2nd.3rd & so on
                                  // level
                if (next != null)
                    next.prev = temp; // if next is null then null.prev will give null pointer exception
            } else { // if no child just move forward
                curr = curr.next;
            }
        }
        return head;
