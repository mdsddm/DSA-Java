...
1 <= k <= nums.length
 */
public class FindAllKDistanceIndices {
    ...
    public static List<Integer> findKDistantIndices(int[] nums, int key, int k) {
        ...
        for (int i = 0; i < nums.length; i++)
            if (nums[i] == key)
                keys.add(i);
        // Step 2: For each index, check if it’s within distance k from any key position
        int j = 0; // pointer for keys
        for (int i = 0; i < nums.length; i++) {
            while (j < keys.size() && keys.get(j) < i - k)
                j++;
            if (j < keys.size() && Math.abs(keys.get(j) - i) <= k)
                ans.add(i);
        }
        return ans;
    }
